...
class ForStatement extends ASTNode {
    ...
    eval(context)
    {
        ...
        let color = context.symbol_table["COLOR"]
...
class AssignExpression extends ASTNode {
    ...
    eval(context) {
        ...
        return context.symbol_table[id] = r
    }
}
...
export class Parser {
    ...
    atom()
    {
        ...
        }else if(is_var && this.is(TokenType.ASSIGN))
        {
            this.match(TokenType.ASSIGN)

            let r = this.expression()
            expr = new AssignExpression(expr, r)
        }
        return expr
    }
